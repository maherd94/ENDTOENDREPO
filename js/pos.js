(function () {
  ...
  function showToast(m, t = 'Action needed') {
    const $t = $('#toast');
    $t.find('.toast-title').text(t);
    $t.find('.toast-msg').text(m);
    $t.prop('hidden', false);
    clearTimeout($t.data('hideTimer'));
    $t.data('hideTimer', setTimeout(() => $t.prop('hidden', true), 2600));
  }
  $(document).on('click', '#toast-close', () => $('#toast').prop('hidden', true));

  function renderProducts() {
    const $g = $('#pos-products').empty();
  ...
  $(document).on('click', '.qty-dec', function () {
    ...
    if (--ci.qty <= 0) delete state.cart[id];
    renderCart();
  });
  ...
  $('#pos-terminal').on('change', function () {
    const opt = this.options[this.selectedIndex];
    const val = $(opt).val();
    state.terminal = val ? {
      terminal_id: $(opt).data('terminal-id') || val,
      id: val,
      label: $(opt).text()
    } : null;
    renderCart();
  ...
  // ---- apis
  function payOnTerminal(terminalId, amountMinor, currency, transactionId, shopperReference) {
    return new Promise((resolve, reject) => {
      $.ajax({
        url: '/apis/pos_pay.php',
        method: 'POST',
        contentType: 'application/json',
        dataType: 'json',
        data: JSON.stringify({ terminalId, amount: { value: amountMinor, currency }, transactionId, shopperReference }),
        success: resolve,
  ...
  function renderPblResult(pbl){
    ...
    if ($.fn.qrcode && url) {
      try { $qr.qrcode({ width: 220, height: 220, text: url }); }
      catch { $qr.text('QR unavailable'); }
    } else $qr.text('QR unavailable');
  ...
  $('#pos-pbl').off('click').on('click', async function(e){
    ...
    try {
      // 1) Create the payment link (request tokenization server-side)
      ...
      // 2) Create the matching local order
      try {
  ...
  function decodeReceiptLines(arr) {
    const lines = [];
    (arr || []).forEach(o => {
      const txt = (o && o.Text) || '';
      const map = Object.fromEntries(txt.split('&').map(p => {
        const [k, v] = p.split('=');
        try { return [k, decodeURIComponent(v || '')]; } catch { return [k, v || '']; }
      }));
      if (map.name && (map.value || map.key)) lines.push(`${map.name}${map.value ? ': ' + map.value : ''}`);
  ...
  function renderTerminalResponse(resp) {
    ...
    if (!r) { $('#pos-terminal-message').html('<div style="color:#b91c1c">Invalid response from terminal</div>'); return; }
    const pr = r.PaymentResponse || {};
    const res = pr.Response || {};
    const result = res.Result || 'Unknown';
    const poiData = pr.POIData || {};
    const paymentResult = pr.PaymentResult || {};
    const amountsResp = paymentResult.AmountsResp || {};
    const instr = paymentResult.PaymentInstrumentData || {};
    const card = instr.CardData || {};
    const masked = card.MaskedPan || '';
    const brand = card.PaymentBrand || '';
  ...
  function extractPaymentDetails(resp) {
    ...
    return {
      ...
      paymentAccountReference: map.PaymentAccountReference || null, // token if returned
      alias: map.alias || null
    };
  }

  function genOrderRef() {
    return 'ord_' + Date.now() + String(Math.floor(Math.random() * 1000)).padStart(3, '0');
  }
  ...
  $('#pos-checkout').on('click', async function (e) {
    ...
    try {
      // PASS SHOPPER REF HERE for tokenization on terminal
      const resp = await payOnTerminal(termId, value, currency, txId, shopperRef);
      if (!resp || !resp.ok) {
      ...
      if (details.ok) {
        ...
        const payload = {
          ...
          paymentAccountReference: details.paymentAccountReference || null,
  ...
  function loadCustomers() {
    return $.getJSON('/apis/customers_list.php').then(res => {
      const $s = $('#pos-customer').empty().append('<option value="">-- Select customer --</option>');
      (res.data || []).forEach(c => {
        const name = c.display_name || capitalize(String(c.email || '').split('@')[0] || '');
        $('<option>')
          .val(c.id)
          .text(`${c.shopper_reference} — ${name}`)
  ...
  function normalizeTerminals(data) {
    const out = [];
    (data || []).forEach(t => {
      if (!t) return;
      if (typeof t === 'string') out.push({ terminal_id: t, label: t });
      else if (typeof t === 'object') {
        const id = t.terminal_id || t.id || '';
        const label = t.label || id;
        if (id) out.push({ terminal_id: id, label });
      }
    });
    return out;
  }
  function loadTerminals() {
    return $.getJSON('/apis/terminals_list.php').then(res => {
      const items = normalizeTerminals(res.data);
      const $s = $('#pos-terminal').empty().append('<option value="">-- Select terminal --</option>');
      items.forEach(t => {
        $('<option>').val(t.terminal_id).text(t.label || t.terminal_id).attr('data-terminal-id', t.terminal_id).appendTo($s);
      });
    }).catch(() => {
      const $s = $('#pos-terminal').empty().append('<option value="">-- Select terminal --</option>');
      ['P400Plus-TEST', 'V400m-TEST'].forEach(id => $('<option>').val(id).text(id).attr('data-terminal-id', id).appendTo($s));
    });
  }
